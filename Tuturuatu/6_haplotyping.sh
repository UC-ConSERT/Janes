...
java -jar ##don't know what this file is:/usr/bin/beagle.28Jun21.220.jar gt=#filtered_vcf_file out=#out_vcf_file_name
...
samtools faidx ${sppdir}ref_genome/Maui_merged_assembly.fa '#tlr_region chr:start-end'| bcftools consensus ${finaldir}Tuturuatu_VariantCalls_final_variants.vcf.gz -M N >> ${finaldir}TLR_variants.fasta
...
do
    ...
    samtools faidx ${sppdir}ref_genome/Maui_merged_assembly.fa #tlr_region | bcftools consensus ${finaldir}Tuturuatu_VariantCalls_final_variants.vcf.gz -s $base -M N -H 1pIu >> ${finaldir}tlr_haplotypes.meg
    echo "#"${counter}"-2" >> ${finaldir}tlr_haplotypes.meg
        #outputs sequence for phased haplotype 2 for one individual
    samtools faidx ${sppdir}ref_genome/Maui_merged_assembly.fa #tlr_region | bcftools consensus ${finaldir}Tuturuatu_VariantCalls_final_variants.vcf.gz -s $base -M N -H 2pIu >> ${finaldir}tlr_haplotypes.meg
    counter=$(($counter+1))
done