...
## I'm pretty sure this is ddone one tlr ata time, so ive done tlr7 for a start


sppdir=~/data/tuturuatu_trial_2/
finaldir=${sppdir}bcf_final/

#bgzip ${finaldir}Tuturuatu_VariantCalls_final_variants.vcf.gz
##oj removed above line and put in previous script 5_4 for future...
...
samtools faidx ${sppdir}ref_genome/Maui_merged_assembly.fa jcf7180002696332:371554-374695 | bcftools consensus ${finaldir}Tuturuatu_VariantCalls_final_variants.vcf.gz -M N >> ${finaldir}TLR_variants.fasta
...
do
    ...
    samtools faidx ${sppdir}ref_genome/Maui_merged_assembly.fa jcf7180002696332:371554-374695 | bcftools consensus ${finaldir}Tuturuatu_VariantCalls_final_variants.vcf.gz -s $base -M N -H 1pIu >> ${finaldir}tlr_haplotypes.meg
    echo "#"${counter}"-2" >> ${finaldir}tlr_haplotypes.meg
        #outputs sequence for phased haplotype 2 for one individual
    samtools faidx ${sppdir}ref_genome/Maui_merged_assembly.fa jcf7180002696332:371554-374695 | bcftools consensus ${finaldir}Tuturuatu_VariantCalls_final_variants.vcf.gz -s $base -M N -H 2pIu >> ${finaldir}tlr_haplotypes.meg
    counter=$(($counter+1))
done