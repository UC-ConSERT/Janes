...
# Imputation
    ...
    do
        ...
        do
            java -jar ${beaglejar} gt=${file} impute=true gp=true ne=${test_ne} nthreads=16 \
                ref=${refvcf} out=${impdir}beagle_imputations/${base}_${test_ne}ne_beagle_imp
            echo ""; echo "Indexing ${base}"
            bcftools index -f --threads 16 ${impdir}beagle_imputations/${base}_${test_ne}ne_beagle_imp.vcf.gz
        done
    done
